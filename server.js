...
app.get('/api/epics', requireAuth, (req, res) => res.json(db.prepare('SELECT * FROM epics ORDER BY round, epic_number').all()));
app.get('/api/assignments', requireAuth, (req, res) => res.json(assignmentView()));
...
app.listen(PORT, '0.0.0.0', () => console.log(`Hackathon system running on http://0.0.0.0:${PORT}`));
